...
describe('AccountBoxController', function () {
  ...
  const SKPTOSHI_UNIT = 100000000
  const accounts = [
    { balance: 10 * SKPTOSHI_UNIT },
    { balance: 15 * SKPTOSHI_UNIT },
    { balance: 5 * SKPTOSHI_UNIT },
    {}
  ]
  ...
  beforeEach(() => {
    module('skelpyclient.components', $provide => {
      $provide.value('SKPTOSHI_UNIT', Math.pow(10, 8))
    })
  ...
  describe('myAccountsBalance()', () => {
    it('sums the balance (in SKP, formatted) of all accounts', function () {
    ...
    context('when the balance of an account changes', () => {
      it('updates the balance', function () {
        ...
        sinon.stub(bindings.accountCtrl, 'getAllAccounts').returns([
          { balance: 1 * SKPTOSHI_UNIT },
          { balance: 17 * SKPTOSHI_UNIT },
          { balance: 1 * SKPTOSHI_UNIT },
          { balance: 1 * SKPTOSHI_UNIT }
        ])
