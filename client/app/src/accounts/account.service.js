;(function () {
  ...
  angular.module('skelpyclient.accounts')
    .service('accountService', ['$q', '$http', 'networkService', 'storageService', 'ledgerService', 'gettextCatalog', 'SKPTOSHI_UNIT', AccountService])
   ...
   * @constructor
   */
  function AccountService ($q, $http, networkService, storageService, ledgerService, gettextCatalog, SKPTOSHI_UNIT) {
    var self = this
    ...
    function createAccount (passphrase) {
      return new Promise((resolve, reject) => {
        const publicKey = skelpy.crypto.getKeys(passphrase).publicKey
        const address = skelpy.crypto.getAddress(publicKey, networkService.getNetwork().version)
    ...
    function savePassphrases (address, passphrase, secondpassphrase) {
      var deferred = $q.defer()
      var tempaddress = skelpy.crypto.getAddress(skelpy.crypto.getKeys(passphrase).publicKey)
      if (passphrase) {
    ...
    function getTransactionLabel (transaction, recipientAddress = null) {
      ...
      if (recipientAddress && transaction.recipientId === recipientAddress && transaction.type === 0) {
        label = gettextCatalog.getString('Receive SKP')
      }
    ...
    function formatTransaction (transaction, recipientAddress) {
      ...
      //   transaction.label = gettextCatalog.getString("Receive Skelpy")
      // }
      }
      ...
      transaction.humanTotal = numberToFixed(transaction.total / SKPTOSHI_UNIT) + ''
    ...
    function verifyMessage (message, publicKey, signature) {
      ...
      var ecpair = skelpy.ECPair.fromPublicKeyBuffer(Buffer.from(publicKey, 'hex'))
      var ecsignature = skelpy.ECSignature.fromDER(Buffer.from(signature, 'hex'))
    ...
    function signMessage (message, passphrase) {
      ...
      var hash = crypto.createHash('sha256')
      hash = hash.update(Buffer.from(message, 'utf-8')).digest()
      var ecpair = skelpy.crypto.getKeys(passphrase)
    ...
    function createTransaction (type, config) {
      ...
      getFees().then(function (fees) {
        var account
        var transaction
        if (type === 0) { // send skelpy
          if (!skelpy.crypto.validateAddress(config.toAddress, networkService.getNetwork().version)) {
          ...
          if (config.amount + fees.send > account.balance) {
            deferred.reject(gettextCatalog.getString('Not enough SKP on your account ') + config.fromAddress)
            return deferred.promise
          }

          try {
            transaction = skelpy.transaction.createTransaction(config.toAddress, config.amount, config.smartbridge, config.masterpassphrase, config.secondpassphrase)
          } catch (e) {
            deferred.reject(e)
            return deferred.promise
          }
          ...
          if (config.ledger) {
            ...
            ledgerService.signTransaction(config.ledger, transaction).then(
              function (result) {
                ...
                transaction.id = skelpy.crypto.getId(transaction)
            ...
            return deferred.promise
          } else if (skelpy.crypto.getAddress(transaction.senderPublicKey, networkService.getNetwork().version) !== config.fromAddress) {
          ...
          if (account.balance < fees.secondpassphrase) {
            deferred.reject(gettextCatalog.getString('Not enough SKP on your account ') + config.fromAddress + ', ' + gettextCatalog.getString('you need at least 5 SKP to create a second passphrase'))
            return deferred.promise
          }
          try {
            transaction = skelpy.signature.createSignature(config.masterpassphrase, config.secondpassphrase)
          } catch (e) {
            deferred.reject(e)
            return deferred.promise
          }
          ...
          if (account.balance < fees.delegate) {
            deferred.reject(gettextCatalog.getString('Not enough SKP on your account ') + config.fromAddress + ', ' + gettextCatalog.getString('you need at least 10 SKP to register delegate'))
            return deferred.promise
          }
          // console.log(config)
          try {
            transaction = skelpy.delegate.createDelegate(config.masterpassphrase, config.username, config.secondpassphrase)
          } catch (e) {
            deferred.reject(e)
            return deferred.promise
          }
          ...
          if (account.balance < fees.vote) {
            deferred.reject(gettextCatalog.getString('Not enough SKP on your account ') + config.fromAddress + ', ' + gettextCatalog.getString('you need at least 1 SKP to vote'))
            return deferred.promise
          }
          try {
            transaction = skelpy.vote.createVote(config.masterpassphrase, config.publicKeys.split(','), config.secondpassphrase)
          } catch (e) {
            deferred.reject(e)
            return deferred.promise
          }
    ...
    function createVirtual (passphrase) {
      var deferred = $q.defer()
      var address = skelpy.crypto.getAddress(skelpy.crypto.getKeys(passphrase).publicKey, networkService.getNetwork().version)
            ...
            if (virtual[folder]) {
              if (arguments.length === 1) {
                if (value === null) {
                  ...
                  virtual[folder].amount = value * SKPTOSHI_UNIT
                }
              } else {
                return virtual[folder].amount === null ? '' : virtual[folder].amount / SKPTOSHI_UNIT
              }
            }
          }
        }
