  ...
  function AccountController (
    ...
    self.closeApp = function () {
      var confirm = $mdDialog.confirm()
        .title(gettextCatalog.getString('Quit SKP Client?'))
    ...
    //   changerService.getMarketInfo(symbol, 'skelpy_SKP').then(function (answer) {
    //     self.buycoin = answer
    //   })

    //   changerService.getMarketInfo('skelpy_SKP', symbol).then(function (answer) {
    ...
    //   changerService.getMarketInfo(self.selectedCoin, 'skelpy_SKP', self.buyAmount / self.buycoin.rate).then(function (rate) {
    //     var amount = self.buyAmount / rate.rate
    //     if (self.selectedCoin.split('_')[1] === 'USD') {
    //       amount = parseFloat(amount.toFixed(2))
    //     }
    //     changerService.makeExchange(self.exchangeEmail, amount, self.selectedCoin, 'skelpy_SKP', self.selected.address).then(function (resp) {
    ...
    //           self.exchangeBuy.receiveCurrency = 'SKP'
    ...
    //   self.exchangeSell.sendCurrency = 'SKP'
    //   var progressbar = $interval(function () {
    ...
    // self.sell = function () {
    //   if (self.exchangeEmail) storageService.set('email', self.exchangeEmail)
    //   changerService.makeExchange(self.exchangeEmail, self.sellAmount, 'skelpy_SKP', self.selectedCoin, self.recipientAddress).then(function (resp) {
    //     accountService.createTransaction(0, {
    //       fromAddress: self.selected.address,
    //       toAddress: resp.payee,
    //       amount: parseInt(resp.send_amount * SKPTOSHI_UNIT),
    ...
    // self.exchangeSkelpyNow = function (transaction) {
    ...
    self.saveFolder = function (account, folder) {
      accountService.setToFolder(account.address, folder, account.virtual.uservalue(folder)() * SKPTOSHI_UNIT)
    }
    ...
    function generateDynamicPalette (callback) {
      ...
      vibrant.from(url).getPalette(function (err, palette) {
        ...
        var isSkelpyJpg = path.basename(url) === 'Skelpy.jpg'
        var primaryColor = isSkelpyJpg ? 'red' : sortObj(darkVibrantRatio)[0]
    ...
    function manageBackgrounds () {
      ...
      function deleteImage (evt, image) {
        ...
        if (image === initialBackground) {
          selectBackground(backgrounds['images']['Skelpy'])
        } else {
    ...
    function showExchangeRate () {
      return self.network.cmcTicker || self.network.token === 'SKP'
    }
    ...
    function showValidateTransaction (selectedAccount, transaction) {
      ...
      $scope.validate = {
        ...
        humanAmount: accountService.numberToFixed(transaction.amount / SKPTOSHI_UNIT).toString(),
        totalAmount: ((parseFloat(transaction.amount) + transaction.fee) / SKPTOSHI_UNIT).toString()
      }
