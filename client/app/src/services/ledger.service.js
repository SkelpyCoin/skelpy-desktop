;(function () {
  ...
  function LedgerService ($q, $http, $timeout, storageService, networkService) {
    ...
    var skelpyjs = require('../node_modules/skelpyjs')
    ...
    function getBip44Accounts (slip44) {
      ...
      async.whilst(
        ...
        function (next) {
          ...
          if (result.address) {
            result.address = skelpyjs.crypto.getAddress(result.publicKey)
    ...
    function recoverBip44Accounts (backupLedgerPassphrase, slip44) {
      var hdnode = skelpyjs.HDNode.fromSeedHex(bip39.mnemonicToSeedHex(backupLedgerPassphrase))
    ...
    function signTransaction (path, transaction) {
      ...
      ipcRenderer.send('ledger', {
        ...
        data: skelpyjs.crypto.getBytes(transaction, true, true).toString('hex'),
        path: path
      })
      return deferred.promise
    }
