;(function () {
  ...
  angular
    .module('skelpyclient.components')
    .controller('AddressbookController', ['$scope', '$mdDialog', 'toastService', 'storageService', 'gettextCatalog', 'accountService', 'SKPTOSHI_UNIT', AddressbookController])

  function AddressbookController ($scope, $mdDialog, toastService, storageService, gettextCatalog, accountService, SKPTOSHI_UNIT) {
    var self = this
    // var contacts
    ...
    self.isAddress = function (address) {
      return require('../node_modules/skelpyjs').crypto.validateAddress(address)
    }
    ...
    self.getStats = function (account, contact) {
      ...
      if (transactions) {
        ...
        if (incomeTx.length > 0) {
          ...
          stats.income.amount = accountService.numberToFixed(incomeAmount / SKPTOSHI_UNIT).toFixed(2)
        }
        ...
        if (expendTx.length > 0) {
          ...
          stats.expend.amount = accountService.numberToFixed(expendAmount / SKPTOSHI_UNIT).toFixed(2)
        }
      }
