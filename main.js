...
const LedgerSkelpy = require('./LedgerSkelpy')
...
function createWindow () {
  ...
  var iconpath = require('path').resolve(__dirname, '/client/skelpy.png')
  ...
  ipcMain.on('ledger', (event, arg) => {
    if (arg.action === 'detect') {
      ...
      if (!ledgercomm) {
        ...
        try {
          let skelpy = new LedgerSkelpy(ledgercomm)
          if (arg.action === 'signMessage') {
            skelpy.signPersonalMessage_async(arg.path, Buffer.from(arg.data)
              .toString('hex'))
            ...
            skelpy.signTransaction_async(arg.path, arg.data)
          ...
          var result = {
            connected: false,
            message: 'Cannot connect to SKP application'
          }
          event.returnValue = result
        }
      }
  ...
  template = [
    {
      ...
      submenu: [
        {
          label: 'About SKP Wallet',
          click: () => openAboutWindow({
            icon_path: `${__dirname}/client/skelpy.png`,
            package_json_dir: __dirname,
            copyright: 'Copyright (c) 2017 SKP and 2017 ArkEcosystem',
            homepage: 'http://skelpy.co/',
            bug_report_url: 'https://github.com/SkelpyCoin/skelpy-desktop/issues'
          })
        },
        { type: 'separator' },
